/********************************************************************************
 *  Copyright (C) 2016 Oay Soluções Inteligentes Ltda (<http://www.oay.com.br>) *
 *                                                                              *
 *  Author Teofanes Enrique <suporte@oay.com.br>                                *
 *                                                                              *
...
main(int argc, char** argv){
    ...
    // Mandamos o Pacote Para ele
    add_gui_init( GTK_WIDGET(gtk_builder_get_object(gtkBuilder, "BoxTable")) );
    
    /* some buttons */
    hbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 4);
    gtk_box_set_homogeneous (GTK_BOX (hbox), TRUE);
    gtk_box_pack_start (GTK_BOX ( gtk_builder_get_object(gtkBuilder, "BoxTable") ), hbox, FALSE, FALSE, 0);

    button = gtk_button_new_with_label ("Adicionar Item");
    g_signal_connect (button, "clicked",
                    G_CALLBACK (add_item), treeview);
    gtk_box_pack_start (GTK_BOX (hbox), button, TRUE, TRUE, 0);

    button = gtk_button_new_with_label ("Remover Item");
    g_signal_connect (button, "clicked",
